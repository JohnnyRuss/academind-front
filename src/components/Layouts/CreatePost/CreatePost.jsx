...
import { setCreatePostIsOpen } from '../../../store/reducers/createPostReducer';
...
function CreatePost({ className }) {
  ...
  */
  ...
  const activateModal = (open) => !updatePostModalIsOpen && dispatch(setCreatePostIsOpen(open));
  ...
  */
  ...
  return (
    <div className={`${styles.createPost} ${className || ''}`}>
      ...
      {createPostIsOpen && !updatePostModalIsOpen && (
        <CreatePostModal
